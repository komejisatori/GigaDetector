...
class Image_api(object):
    def __init__(self, resize_shape=img_cfg['resize_shape'], img_path=None,
                 ms_resize_shape=img_cfg['ms_resize_scale']):
        """
        ...
        self.name = img_path.split('.')[0].split('/')[-1]
        try:
        ...
        if self.image is None:
            ...
            print("[image error] image {} not loaded".format(img_path))
        else:
    ...
    def nms_all_result(self):
        if img_cfg['nms']:
            ...
            for i in keep[0:count]:
                new_result.append(self.result[i].numpy().tolist())
    ...
    def draw(self, path=None):
        ...
        for i in range(self.result.shape[0]):
                ...
                '''
                cv2.rectangle(origin_image,
                        (int(self.result[i,0]), int(self.result[i,1])-30),
                        (int(self.result[i,0])+100, int(self.result[i,1])),
                        [0,0,255], -1)
                cv2.putText(origin_image, '%.2f'%self.result[i, 4], 
                        (int(self.result[i,0])-1,int(self.result[i,1])+2),cv2.FONT_HERSHEY_SIMPLEX,
                        1.2,[255,255,255],2)
                '''
        cv2.imwrite('./' + self.name + '.jpg', origin_image)
    ...
    def expand(self, l=0, t=0, b=0, r=0):
        ...
        return self.image[max(0, y1 - t):min(y2 + b, self.resize_shape[1]),
               max(x1 - l, 0):min(x2 + r, self.resize_shape[0]), :]
