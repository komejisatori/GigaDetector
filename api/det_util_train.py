...
PREFIX = '/home/wangzerun/giga'
DETECTOR_TYPE = 'retina_net' #faster_rcnn cascade_rcnn retina_net
MODEL_CFG_PRETRAIN = {
...
MODEL_PATH_FINETUNE = {
    ...
    'retinanet': PREFIX + '/train/work_dirs/retinanet_r101_fpn_1x_newdataset/epoch_20.pth'
}
JSON_PATH = [
    PREFIX + '/data/label/gt_human_bbox_all.json',
    PREFIX + '/data/label/gt_human_bbox_all_part2_v2.json'
]
TEST_LIST_PATH = PREFIX + '/api/train_list.txt'
TEST_LIST = []
EVAL_LIST = []
with open(TEST_LIST_PATH, 'r') as f:
    while True:
        ...
        if not lines:
            break
        TEST_LIST.append(PREFIX + '/data/frame/train/' + lines.split('\n')[0])
...
def det_giga_full(img_root_dir, detector_type, conf_thres):
    ...
    detfilename = 'traindet' +'_'+DETECTOR_TYPE + file_prefix + '.txt'
...
if __name__ == '__main__':
    ...
    if PRETRAIN_MODEL:
        ...
        file_prefix = ''
    evaluate_det_result('/home/wangzerun/giga/traindet_' + DETECTOR_TYPE + file_prefix + '.txt', JSON_PATH[0], JSON_PATH[1], img_files_list)
