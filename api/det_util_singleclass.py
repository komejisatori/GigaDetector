...
from data.dataloader.coco_converter import result_to_coco_v2_single, convert_to_coco
...
with open(TEST_LIST_PATH, 'r') as f:
    ...
    while True:
        ...
        if lines.split('_')[1].split('_')[0] == CLASS:
            if not started:
                START = COUNT
                started = True
            else:
...
def evaluate_det_result(prediction_txt_path, gt_json_path_1, gt_json_path_2, img_files_list):
    ...
    result_to_coco_v2_single(START, END, prediction_txt_path, output=os.path.join(PREFIX, 'api/result'))
    eval_dict = {}
    eval_names = []
    eval_labels = []
...
def det_giga_full(img_root_dir, detector_type, conf_thres):
    ...
    detfilename = 'det' +'_'+DETECTOR_TYPE + file_prefix + '.txt'
